...
out_dir = os.path.join(project_dir, "density_analysis/")
ica_checkpoint_location = os.path.join(project_dir, "ica_outputs/ica_chk-30000")
density_checkpoint_location = os.path.join(project_dir, "density_outputs/density_chk-10000")
log_interval = 10
...
with tf.Session(graph=graph) as sess:
  ...
  ica_saver.restore(sess, ica_checkpoint_location)
  density_saver.restore(sess, density_checkpoint_location)

  epoch_order = np.arange(epoch_size)
  data_batch = dataset[epoch_order, ...]
  ...
  for v_step in range(num_v_steps):
    [_, v_eval, v_gradient_eval, likelihood_eval] = sess.run([step_v, v, dv,
      likelihood], feed_dict)
    v_inference.append(v_eval)
    gradients.append(v_gradient_eval)
    likelihoods.append(likelihood_eval)
    pf.save_activity_hist(v_gradient_eval, num_bins="auto",
      title="V Gradient Histogram "+str(v_step).zfill(3),
      save_filename=out_dir+"v_grad_hist_"+str(v_step).zfill(3))
    pf.save_activity_hist(v_eval, num_bins="auto",
      title="V Activity Histogram "+str(v_step).zfill(3),
  ...
  [a_eval, b_eval] = sess.run([a, b], feed_dict)
  ...
  np.savez(out_dir+"a_weight_matrix.npz", data=a_eval)
  np.savez(out_dir+"b_weight_matrix.npz", data=b_eval)
